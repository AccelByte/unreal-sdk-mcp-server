...
void UAuthEssentialsSubsystem::OnLoginComplete(int32 LocalUserNum, bool bLoginWasSuccessful, const FUniqueNetId& UserId, const FString& LoginError, const FAuthOnLoginCompleteDelegate OnLoginComplete)
{
	if (bLoginWasSuccessful)
	{
		...
		// Query player achievements after successful login
...
void UAuthEssentialsSubsystem::QueryPlayerAchievements(const FUniqueNetId& UserId)
{
	// Get Online Subsystem
	IOnlineSubsystem* OnlineSubsystem = Online::GetSubsystem(GetWorld());
	if (!OnlineSubsystem)
	{
		UE_LOG_AUTH_ESSENTIALS(Warning, TEXT("Cannot query achievements. Online subsystem is invalid."));
		return;
	}
	...
	{
		UE_LOG_AUTH_ESSENTIALS(Warning, TEXT("Cannot query achievements. Achievements interface is invalid."));
		return;
	}

	UE_LOG_AUTH_ESSENTIALS(Log, TEXT("Querying player achievements for user: %s"), *UserId.ToString());

	// Query user's achievements (includes both unlocked and in-progress)
	AchievementsInterface->QueryAchievements(
		UserId,
		FOnQueryAchievementsCompleteDelegate::CreateUObject(this, &UAuthEssentialsSubsystem::OnQueryAchievementsComplete)
	);
}

void UAuthEssentialsSubsystem::OnQueryAchievementsComplete(const FUniqueNetId& UserId, bool bWasSuccessful)
{
	if (!bWasSuccessful)
	{
		UE_LOG_AUTH_ESSENTIALS(Warning, TEXT("Failed to query achievements for user: %s"), *UserId.ToString());
		return;
	}

	UE_LOG_AUTH_ESSENTIALS(Log, TEXT("Successfully queried achievements for user: %s"), *UserId.ToString());
	...
	// Get cached achievements for this player
	TArray<FOnlineAchievement> PlayerAchievements;
	EOnlineCachedResult::Type Result = AchievementsInterface->GetCachedAchievements(UserId, PlayerAchievements);

	if (Result == EOnlineCachedResult::Success)
	{
		UE_LOG_AUTH_ESSENTIALS(Log, TEXT("Found %d achievements for player"), PlayerAchievements.Num());

		// Log each achievement's details
		for (const FOnlineAchievement& Achievement : PlayerAchievements)
		{
			UE_LOG_AUTH_ESSENTIALS(Log, TEXT("  Achievement: %s - Progress: %.2f%% - Un"),
				*Achievement.Id,
	...
	{
		UE_LOG_AUTH_ESSENTIALS(Warning, TEXT("Failed to get cached achievements"));
	}
}
